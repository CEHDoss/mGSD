...
merge(config.macros,{
  ...
  }, convertScratchToAction: { handler: function(place,macroName,params,wikifier,paramString,tiddler) {
  ...
  }, convertScratchToProject: { handler: function(place,macroName,params,wikifier,paramString,tiddler) {
      if (tiddler.tags.contains('Scratch')) {
  ...
  }, scratchQuickAdd: { handler: function(place,macroName,params,wikifier,paramString,tiddler) {
      var shortHand = {
        Action : {
          'W' : 'Waiting For',
          'N' : 'Next',
      ...
      if (tiddler.tags.contains('Scratch')) {
        createTiddlyButton(place, "quick add", "create these items", function(e) {
          ...
          for (var i = 0; i < lines.length; i++) {
            ...
            var fields_and_text = lines[i].trim().split(/[|;]{2}/);
            // anything after || is to become tiddler content
            ...
            if (!fields[0] || fields[0].trim() == "") {
              ...
              if (title.substr(0, 1) != '.') {
                ...
                if (store.tiddlerExists(title))
                  ...
                  store.saveTiddler(title, title, tiddlerText ? tiddlerText : "", // content
                  config.options.txtUserName, new Date(), fields, // tags
                  null // extra fields
                  );
              } else {
                //alert("action "+title);
                ...
                if (currentProject.trim() != "")
                  fields.push(currentProject);
                // make it in this project
